    ...
    public class UserController : ControllerBase
    {
        ...
        public IActionResult Contact_add(UserToAddDTO Useradd)
        {
            try
            {
                ...
                string requeteSQL = @"select * from ctl_user_add(" + "'" + Useradd.lastname+ "'," + "'"+Useradd.firstname+ "'," + "'"+ Useradd.mobile + "'," + "'" + Useradd.email + "'," + "'" + Useradd.address + "','" + Useradd.pwd+ "'," + "'" + Useradd.statuts+ "'," + "'" + Useradd.entry_date+ "'," + "'" +Useradd.picture+"')";
...

            {
                try
                {
                    ...
                    if (!UserReader.HasRows)
                    {
                        ...
                        npgsqlCommand.Dispose();
                        npgsqlConnection.Close();
                        return Ok(new DataResponse<UserListToUpdateReturnDTO>(false, "User EMPTY", "201", UserToUpdateReturnDTO));
                    }
                    ...
                    return Ok(new DataResponse<UserListToUpdateReturnDTO>(false, "", "201", UserToUpdateReturnDTO));
